...
public class Day05_1 {
	...
	public static void main(String[] args) {
		...
		int 변수 =10;  //int : 4바이트 =>
			...
			// 변수 / 배열 -> 메모리
				//1. 메모리는하나의 자료형의 크기만큼 주소값[16진수: 생성]
				//2. 컴퓨터는 주소값을 통한 메모리 접근

		
		// 1차원 배열 : 배열의길이
		int[] 배열 = new int[3];	//int : 4바이트 *10 => 배열의크기는 40바이트
			배열[0] = 80;
		// 2차원 배열 : 배열의 [가로길이] [새로길이] // 4바이트 * 2 * 3 => 24바이트
		int[][] 배열2 = new int[2][3];
		...
		//문제1 : 반복문을 이용한 0~49까지 첫번때 행에 저장
							//50~99까지는 두번쨰 행에 저장하는 2차원배열 선언하고 출력
		int[][] 배열4 = new int[2][50];
		
		for(int i = 0 ; i<100; i++) {
			
			if(i<50) 배열4[0][i]=i; //해당 i 인덱스 i 값 넣기
			else 배열4[1][i-50]=i;
		}
		for(int i= 0 ; i<100; i++) {
